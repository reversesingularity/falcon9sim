// visualization.js - Three.js 3D Visualization Engine (FIXED)
...
class Visualization {
    constructor(canvasId) {
        ...
        this.rotationX = 0;
        this.rotationY = 0;
        this.cameraMode = 'overview'; // Add camera mode tracking
    ...
    init() {
        // Scene setup with realistic sky gradient
        this.scene = new THREE.Scene();
        this.scene.background = new THREE.Color(0x0a1628);
        this.scene.fog = new THREE.FogExp2(0x0a1628, 0.00008);
    ...
    createFalcon9() {
        ...
        // Set initial position on launch pad
        this.falcon9.position.set(0, 0.04, 0); // 40m above ground in km
    ...
    updateRocketPosition(position, rotation) {
        if (this.falcon9) {
            ...
            if (this.rocketLight) {
                ...
                this.rocketLight.intensity = window.simulation ? window.simulation.throttle * 1.5 : 0;
            }
        }
    }
    ...
    createExhaustEffect(intensity) {
        ...
        if (intensity > 0.1 && this.falcon9 && this.enginePositions) {
            ...
            const numEngines = intensity > 0.7 ? 9 : 3;
    ...
    updateCamera() {
        ...
        if (this.cameraMode === 'chase' && this.falcon9) {
            // Follow the rocket
            const rocketPos = this.falcon9.position;
            const offset = new THREE.Vector3(
                Math.sin(this.rotationY) * this.cameraDistance,
                50 + Math.sin(this.rotationX) * this.cameraDistance * 0.5,
                Math.cos(this.rotationY) * this.cameraDistance
            );
            
            this.camera.position.lerp(rocketPos.clone().add(offset), 0.1);
